...
app.post('/register', async (req, res) => {
  try {
    ...
    let result = await client.db('Lab03').collection('Register').insertOne({
...
app.get('/login', async (req, res) => {
  try {
    let result = await client.db("Lab03").collection("Register").findOne({
      username: req.body.username
    });
...
app.patch('/updateUser', async (req, res) => {
  try {
    ...
    const user = await client.db('Lab03').collection('Register').findOne({ username: currentUsername });
    if (!user) {
    ...
    const updateResult = await client.db('Lab03').collection('Register').updateOne(
...
app.delete('/deleteUser/:username', async (req, res) => {
  try {
    ...
    const deleteResult = await client.db('Lab03').collection('Register').deleteOne({ username });
